...
- **`:core:data` hides `:core:network`** (`implementation`, not `api`). Retrofit types and wire models must not surface in its public API — keep API-facing constructors `internal`. Wire → domain mapping lives in `core/data/mapper/`. Use the `core-data-repository` skill for repositories, data sources, domain models and mappers — it carries that boundary rule, the layering (domain model → mapper → internal data source → public repository), and the fake-based test patterns.
